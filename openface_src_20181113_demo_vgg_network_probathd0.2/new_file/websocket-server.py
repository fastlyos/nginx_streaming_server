...
openfaceNewModelDir = os.path.join(fileDir, '..', '..', 'new_file')
...
parser.add_argument('--networkModel', type=str, help="Path to Torch network model.",
                    ...
                    default=os.path.join(openfaceModelDir, 'VGG_FACE.t7'))
...
align = []
#net = openface.TorchNeuralNet(args.networkModel, imgDim=224, cuda=args.cuda)
...
for qq in range(0, number_of_cpu_net):                              
    ...
    #cpunet.append(openface.TorchNeuralNet(args.networkModel, imgDim=224, cuda=False))
...
class Face:
    ...
    def __init__(self, rep, identity, imgdata):
        self.rep = rep
...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    def __init__(self):
        ...
        self.use_dlib_confirmation = True

        self.rescueMaxtime = 1
        ...
        self.num_of_cam = 2
        self.nn_processed_bbs_max = 1
        self.ReInitializedTrackingVars()
        self.buffer_to_learn = 0
        ...
        self.ui_size_x = 928 # 816 #640 #816 #160 #384 #816 # 320
        self.ui_size_y = 522 # 459  #360 #459 # 90 # 216 #459 #180
        self.camera_size_x = 2560 #1920
        self.camera_size_y = 1440 #1080

        self.queuesize = 2
        self.min_bb_width_to_process = 72
        self.BestMatchSimilarityScoreThd = 0.2
        self.BestMatchSimilarityScoreThd_reconfirm = 0.1
    ...
    def processImgToEncode(self, cam_id):
        ...
        while True:
            counter=counter+1
            if not self.bufferframe[cam_id].empty() and not self.loadingstate:
                start = time.time()
                ...
                if args.verbose and counter%time_log_freq==0:
                    print("[cam{}] cv2.resize took {} seconds.".format(cam_id, time.time() - start)) 
                    start = time.time()                
                ...
                if args.verbose and counter%time_log_freq==0:
                    print("[cam{}] encode image took {} seconds.".format(cam_id, time.time() - start))    

                #self.encodedimage[cam_id] = content    
                # 
                if self.encodedimage[cam_id].full():
                    self.encodedimage[cam_id].get()
                self.encodedimage[cam_id].put(content)        
    ...
    def queryFrame(self, cam_id, path):
        ...
        while True:
            ...
            if self.cap[cam_id].isOpened():
                ...
                # if args.verbose and counter%time_log_freq==0:
                #     print("cam id {} timestamp is {}".format(cam_id, self.cap[cam_id].get(cv2.CAP_PROP_POS_MSEC)))   
        ...
        while True:
            ...
            if not self.tolearnimgqueue.empty():
                ...
                if ctx is None:
                    ...
                    self.images[phash] = Face(self.trainingRep[index], identity, alignedFace)
    ...
    def Determine_identity_by_rep(self, cam_id, rep, isReConfirm=False):
        ...
        BMS_thd=0
        if isReConfirm:
            BMS_thd = self.BestMatchSimilarityScoreThd_reconfirm      
        else:
            BMS_thd = self.BestMatchSimilarityScoreThd 
        ...
        if len(self.people) == 0:
            ...
            if self.svm[cam_id]:
                ...
                #identity = self.svm[cam_id].predict([rep])[0]

                #also need to double confirm with the probability of each class
                prob = self.svm[cam_id].predict_proba([rep])[0]
                #print("[cam{}] svm prob of each class: {}".format(cam_id, ' '.join(str(e)[:6] for e in prob)))

                #identity = np.argmax(prob)
                maxindex = np.argsort(prob)[::-1]

                for z in range(0, showmax):
                    print("[cam{}] max{} svm prob is {} , of identity {} at class index {}, called [{}]".format(cam_id, z, prob[maxindex[z]], self.identity_ofppl[maxindex[z]], maxindex[z], self.people[maxindex[z]]))
                
                #since predict_proba may be different from predict, according to scikit website, just use result of predict_proba for debug
                identity = self.identity_ofppl[maxindex[0]]
            ...
            #     #start = time.time()
            #     #when added person >1, the identity is the index return by svm
            #     identity2 = self.knn[cam_id].predict([rep])[0]

            #     #also need to double confirm with the probability of each class
            #     prob2 = self.knn[cam_id].predict_proba([rep])[0]
            #     print("[cam{}] knn prob of each class: {}".format(cam_id, ' '.join(str(e)[:6] for e in prob2)))

            #     maxindex = np.argsort(prob2)[::-1]

            #     for z in range(0, showmax):
            #         print("[cam{}] max{} knn prob is {} , of identity {} at class index {}, called [{}]".format(cam_id, z, prob2[maxindex[z]], self.identity_ofppl[maxindex[z]], maxindex[z], self.people[maxindex[z]]))
            ...
            #     #     identity = -1
            #     #     print("Top prob < {}, not so sure is one of the trained person, treat as unknown".format(prob_thd))
            #     #print("[cam{}] knn predict took {} seconds.".format(cam_id, time.time() - start))
            ...
            if identity >= 0:
                if self.mean and self.std:
                    ...
                    BestMatchSimilarityScore = round_to_1(max(prob))

                    #BestMatchSimilarityScore = prob
                    # print("[cam{}] BestMatchSimilarityScore is {}".format(cam_id, BestMatchSimilarityScore))

                    #BestMatchSimilarityScoreThd = 0.8
                    if BestMatchSimilarityScore < BMS_thd:
                    ...
                    #     print("[cam{}] The case is pass!".format(cam_id))

                    # else:

                        #print("[cam{}] BestMatchSimilarityScore <= {}, not so sure is one of the trained person, treat as unknown".format(cam_id, BestMatchSimilarityScoreThd))
                        identity = -1
    ...
    def matching(self, cam_id, bb, bbs_ref, identitylist):
        ...
        if len(bbs_ref)==0:
            #print("[cam{}] len(bbs_ref)==0, tracking fail!".format(cam_id))
            return (index, min_dist, NeedReConfirm)
        ...
        if match_count >= 2:
            NeedReConfirm = True
            print("[cam{}] match_count = {}, possible matching boundingbox num > 1, need re-confirm, best matching is identity {}".format(cam_id, match_count, identitylist[index]))            
        ...
        if index >= 0:
            ...
            if bb_ratio < bb_ratio_thd or bb_ratio > 1.0/bb_ratio_thd:
                NeedReConfirm = True
                print("[cam{}] bb_ratio = {} exceed limit (<{} / > 1/{}), need re-confirm".format(cam_id, bb_ratio, bb_ratio_thd, bb_ratio_thd))

        if index >= 0 and min_dist > dist_thd: # or min_dist == 0.0:
            if min_dist != 9999:
    ...
    def onMessage(self, payload, isBinary):
        ...
        if self.counter < 30 or self.counter % 500 == 0:
        ...
        if msg['type'] == "ALL_STATE":
            ...
            #cam_id = msg['id']
            hasstatemsg = msg['hasstatemsg']
            if hasstatemsg != "None":
                self.loadingstate = True
                print("Stop all encode and process frame! ready to loadstate!")
            ...
            if learnmsg != "None":
                ...
                Mexisting = msg['learnmsg']['MergeExisting']
                self.learn_unknown_person(newpersonname, isWithAug, img_list_dataurl, Mexisting)
            ...
            aug_all_existing = msg['aug_all_existing']
            if aug_all_existing != "None":
                self.Augment_All_Existing()
                print("Augment all existing persons!")
                print("==================================================================")

            #Get all the learnt images and so all to the client at one time
            maxlearnmasg = 100
            learnmsgs = []
            while not self.learnimgqueue.empty() and len(learnmsgs) < maxlearnmasg:
            ...
            for qx in range(0, len(self.video_stream_path)):
                if not self.processed_content[qx].empty():
                    pcontent.append((qx, self.processed_content[qx].get_nowait()))
                # else:
                #     print("self.processed_content[{}] is empty!".format(qx))

            #print("debug x")
            pencodedimage = []
            for qz in range(0, len(self.video_stream_path)):
                if not self.encodedimage[qz].empty():
                    pencodedimage.append(self.encodedimage[qz].get_nowait())            

            if len(pencodedimage) == 2 and not pencodedimage[0] is None and not pencodedimage[1] is None:
                msg = {
                    "type": "ANNOTATED",
                    "content": pcontent,
            ...
            # elif not pencodedimage[0] is None:
            #     msg = {
            #         "type": "ANNOTATED",
            #         "content": pcontent,
            #         "fps": round_to_1(1/self.lastframetime[0]),
            ...
            # if args.verbose and self.counter%time_log_freq==0:
            #     print("msg==frame took {} seconds.".format(time.time() - start))     
            ...
            if len(self.cap) > 0:
                ...
                for zzz in range(0, len(self.video_stream_path)):
                    ...
                    self.encodedimage.append(Queue.Queue(maxsize=self.queuesize))
                    self.prev_bbs_isused.append(None)
                    self.lastframetime.append(0.025)
                    self.svm.append(None)
                    self.knn.append(None)
    ...
    def loadState(self, jsImages, training, jsPeople, jsIdentity):
        ...
        for jsImage in jsImages:
            ...
            #if rep == "0" or counter==0:
            #if rep == "0":
            if len(rep) < 129:
                #print("The image {} of identity {} has old rep, need to recalculate!".format(counter, jsImage['identity']))
                ...
                rep = None
                if self.gpu_mutex.acquire():
                    rep = net.forward(photo)
                    self.gpu_mutex.release()

                # if counter==0:
                #     print("rep under cpunet:")
                #     print(rep)

            self.images[h] = Face(np.array(rep), jsImage['identity'], jsImage['image'])
            counter = counter + 1
    ...
    def getData(self):
        ...
        for img in self.images.values():
            ...
            if img.identity in self.imagenumber_foreachide:
                self.imagenumber_foreachide[img.identity] += 1
            else:
                self.imagenumber_foreachide[img.identity] = 1   
        ...
        if len(X) > 0:
            print("Length of feature vector: {}".format(len(X[0])))
        ...
        print("Identities have calulcated their class mean and std")
    ...
    def Augment_All_Existing(self):
        self.loadingstate = True
        aug_half_num = 50
        threadcounter = 0

        # traverse in reversed order so the UI can be seen quickly
        for idx in reversed(self.identity_ofppl):
            if idx in self.imagenumber_foreachide and self.imagenumber_foreachide[idx] < aug_half_num*2:

                aug_ratio = (aug_half_num*2)/self.imagenumber_foreachide[idx]
                ...
                if augmentation_num > 0:
                    for img in self.images.values():
                        if img.identity == idx:
                            ...
                            head = "data:image/jpeg;base64,"
                            if img.imgdata.startswith(head):
                                data = img.imgdata[len(head):]
                                data2 = urllib.unquote(data)
                                imgdata = base64.b64decode(data2)
                                ...
                                processframethread = threading.Thread(target=self.AugmentThread, args=(threadcounter, idx, augmentation_num, photo))                         
                                processframethread.start()      
                                threadcounter += 1
                            
                            else:
                                print("identity {} has one img whose imgdata not start with data:image/jpeg....".format(idx))
                
                else:
                    print("No need to augment identity {}, called {}, he/she has enough samples".format(idx, self.people[self.identity_ofppl.index(idx)])) 
                #print("Augment identity {} to close to {}".format(idx, aug_half_num*2)) 

            else:
                print("No need to augment identity {}, called {}, he/she has enough samples".format(idx, self.people[self.identity_ofppl.index(idx)])) 

        print("Finish Augment all existing, time to train...") 
        time.sleep(1)
        self.trainFace()

        #let client to process to message
        #time.sleep(20)
        self.loadingstate = False


    def learn_unknown_person(self, newpersonname, isWithAug, img_list_dataurl, MergeExistingIdx):
        identity = 0

        #print("MergeExistingIdx= {}".format(MergeExistingIdx))
        if MergeExistingIdx == -1:
        ...
        self.buffer_to_learn += len(img_list_dataurl) 
        if self.buffer_to_learn > 200:
            time.sleep(1)
            #thread = threading.Thread(target=self.trainFace, args=())
            self.trainFace()
            #thread.start()       
            ...
            print("self.buffer_to_learn={}, not trainface yet".format(self.buffer_to_learn))
    ...
    def trainFace(self):
        ...
        print("trainFace()")
        print("Total number of trained person: {}".format(len(self.people)))
        # print("List of trained person and their identity index:")
        # for q in range(0, len(self.people)):
        #     print("[{}, {}]".format(self.people[q], self.identity_ofppl[q]))
        ...
        for key, value in self.rep_of_each_class.iteritems():
            if len(value)>=5:
                ...
                print("Check: identity {} has {} <5 training samples, wont add to svm".format(key, len(value)))

        if d is None:
            for zz in range(0, len(self.video_stream_path)):
                self.svm[zz] = None
                self.knn[zz] = None
            print("No data, skip svm and knn")
            return
        else:
            start = time.time()
            ...
            #print(y)
            #self.svm[0] = GridSearchCV(SVC(C=1, probability=True, kernel='linear'), param_grid, cv=5).fit(X, y)
            self.svm[0] = SVC(C=1.0, probability=True, kernel='linear').fit(X, y)
            for zz in range(1, len(self.video_stream_path)):
                #self.svm[zz] = SVC(C=1.0, probability=True, kernel='linear').fit(X, y)
                self.svm[zz] = copy.deepcopy(self.svm[0])

            print("train svm took {} seconds.".format(time.time() - start))
            start = time.time()

            #neighbour_num = min(5, len(y))

            # for zz in range(0, len(self.video_stream_path)):
            #     self.knn[zz] = KNeighborsClassifier(n_neighbors=neighbour_num)
            #     self.knn[zz].fit(X, y)
    ...
    def processFrame(self, cam_id):
        ...
        while True:
            ...
            #bbs = align[cam_id].getAllFaceBoundingBoxes(rgbFrame_gray)
            ...
            # First, kill bbs which are too small or not passing dlib confirmation, so wont involve in matching
            true_bbs = []
            for bb in bbs:
                bb_width = bb.right()-bb.left()
                
                if bb_width < self.min_bb_width_to_process:
                    continue

                if self.use_dlib_confirmation:
                    dlib_margin = 0
                    rgbFrame_roi = rgbFrame[max(bb.top()-dlib_margin, 0):min(self.camera_size_y-1, bb.bottom()+dlib_margin),max(bb.left()-dlib_margin, 0):min(self.camera_size_x-1, bb.right()+dlib_margin)]
                    rgbFrame_roi_resize = None

                    desired_width = 96 + 2.0*dlib_margin
                    resize_ratio = desired_width/(bb_width + 2.0*dlib_margin)
            ...
            for idxx, bb in enumerate(true_bbs):
                ...
                #if self.training:
                # the size is a multiple of 24, so 24, 48, 72, 96, ...
                # if bb_width < self.min_bb_width_to_process:
                #     #print("[cam{}] bb width= {} < {}, only accept big enough heads to be processed".format(cam_id, bb_width, self.min_bb_width_to_process))
                ...
                if (not self.training and len(self.prev_bbs[cam_id])>0 and 
                    ...
                    if not identity in self.unknowntraining_list:
                        if identity != -1:
                            ...
                            if not identity in self.tracked_list_of_ppl[cam_id] or matchingresult[2] == True:
                                # double confirm whether it is real unmatch with any person in the database
                                # becoz it is possible that it is a miskake to recongize a real person into unknown
                                alignedFace = align[cam_id].align(args.imgDim, rgbFrame, bb, landmarkIndices=openface.AlignDlib.OUTER_EYES_AND_NOSE)  
                                ...
                                #print("Double check if this rep can match with any record in the database...")
                                (ide, sscore) = self.Determine_identity_by_rep(cam_id, rep, matchingresult[2])

                                if ide == identity:
                                    if not identity in self.tracked_list_of_ppl[cam_id]:
                                        isNewlyDetect = True
                                        print("[cam{}] confirmed a newly detected face! update the result table====>".format(cam_id))
                                        self.tracked_list_of_ppl[cam_id].append(identity)   
                                        #This one isTracking is false because just use to add result table, not show bb, since may not
                                        #be at the same time frame

                                    else:
                                        print("[cam{}] tracking not success but re-confirmation successful".format(cam_id))
                                        isTracking = True

                                else:
                                    print("[cam{}] confirmation fails, dont add to result table".format(cam_id))

                            else:
                                isTracking = True
                            
                            #if isTracking:
                                #print ("[cam{}] ===> [{}] is detected in tracking! its identity is {}".format(cam_id, name, identity))
                    ...
                    bb_width*inv_scale, identity, isNewlyDetect, BestMatchSimilarityScore, BB_dataurl, isTracking))
                    #continue
                ...
                #when tracking fails or if it is in active learning mode
                    ...
                    if nn_processed_bbs >= self.nn_processed_bbs_max:
                        print("[cam{}] quota for net.forward() is consumed, treat this bb in next frame!".format(cam_id))
                        identitylist.append(-99) # -99 means its not going to be tracked
                        replist.append(None)
                        scorelist.append(0.0)
                        NameAndBB.append(("Unknown", bb.left()*inv_scale, bb.top()*inv_scale, (bb.right()-bb.left())*inv_scale,
                        (bb.bottom()-bb.top())*inv_scale, -1, False, BestMatchSimilarityScore, BB_dataurl, False))                        
                        continue
                    ...
                    if self.training:
                        ...
                        bb_width*inv_scale, bb_width*inv_scale, identity, isNewlyDetect, BestMatchSimilarityScore, BB_dataurl, False))                     
                        ...
                        #rep = cpunet[cam_id].forward(alignedFace)
                        ...
                        if identity == -1:
                            name = "Unknown"
                            #print("[cam{}] An unknown is newly detected! Add to result table====>".format(cam_id))
                            isNewlyDetect = True                            
                        else:
                            name = self.people[self.identity_ofppl.index(identity)] #+ ", " + str(round_to_1(prob[0]*100)) + "%"
                            print ("[cam{}]  ==> [{}] is detected the first time! its identity is {}".format(cam_id, name, identity))

                        NameAndBB.append((name, bb.left()*inv_scale, bb.top()*inv_scale, (bb.right()-bb.left())*inv_scale,
                        (bb.bottom()-bb.top())*inv_scale, identity, isNewlyDetect, BestMatchSimilarityScore, BB_dataurl, False))
            ...
            if not self.training and not StopUpdatePrev:
                ...
                for idx, val in enumerate(self.prev_identity[cam_id]):
                    ...
                    if not val in identitylist and val > -1: # and val in self.unknowntraining_list:
                        ...
                        if not val in self.rescuetimes[cam_id] or val in self.rescuetimes[cam_id] and self.rescuetimes[cam_id][val] < self.rescueMaxtime:
                            true_bbs.append(self.prev_bbs[cam_id][idx])
                            ...
                            print("[cam{}] cannot rescue identity {} anymore, called [{}]".format(cam_id, val, self.people[self.identity_ofppl.index(val)]))
                            if val in self.tracked_list_of_ppl[cam_id]:
                                self.tracked_list_of_ppl[cam_id].remove(val)
                    
                    elif val in identitylist and val > -1:
                        self.rescuetimes[cam_id][val]=0

                self.prev_bbs[cam_id] = true_bbs
            ...
            if self.processed_content[cam_id].full():
                lastBB_record = self.processed_content[cam_id].get()
            
            # Make sure all the newly added person will be transmit to the client
            if not lastBB_record is None:
                for bbb in lastBB_record:
                    if bbb[6] == True and bbb[5] >= 0:
                        NameAndBB.append(bbb)
                        #print("[cam{}] Transfer old bb with new person to latest NameAndBB".format(cam_id))
