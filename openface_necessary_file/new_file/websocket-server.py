...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def processFrame(self, dataURL, identity):
        ...
        rgbFrame_org = np.zeros((300, 400, 3), dtype=np.uint8)
        rgbFrame_org[:, :, 0] = buf[:, :, 2]
        rgbFrame_org[:, :, 1] = buf[:, :, 1]
        rgbFrame_org[:, :, 2] = buf[:, :, 0]

        rgbFrame = cv2.resize(rgbFrame_org, (0,0), fx=0.5, fy=0.5)
        ...
        if not self.training:
            ...
            self.sendMessage(json.dumps(msg))
            if args.verbose:
                print("Send back the ANNOTATED image took {} seconds.".format(
                    time.time() - start))
                start = time.time()

        if args.verbose:
            print("One frame took {} seconds.".format(
