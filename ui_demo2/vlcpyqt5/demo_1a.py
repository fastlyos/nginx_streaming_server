...
qtCreatorFile = "demo_1a.ui" # Enter file here.
...
class MyThread(QThread):
    ...
    def run( self ):
        while True:
            hightotal = int(subprocess.check_output('sh ../get_count.sh ' + video_src_server_ip + ' high', shell=True))
            lowtotal = int(subprocess.check_output('sh ../get_count.sh ' + video_src_server_ip + ' low', shell=True))   
            
            client_num =  int(subprocess.check_output('sh ../client_count.sh ' + video_src_server_ip, shell=True))
