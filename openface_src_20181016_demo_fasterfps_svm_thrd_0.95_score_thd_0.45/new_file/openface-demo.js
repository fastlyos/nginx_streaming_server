...
function sendFrameLoop()
{
    ...
    //     //console.log(dataURL);
        ...
        var msg = {'type': 'FRAME', 'learnmsg': learnmsg, 'statemsg': statemsg};
        socket.send(JSON.stringify(msg));
...
function redrawResultTable()
{
    ...
    for(var p =result_table.length-1; p>=0; p--)
    {
        if(result_table[p].bestmatchidentity>=0 && hu_counter <= matched_result_max_show)
        {
            ...
            {
                hu += ( "<img src=" + images[result_table[p].recordimageindex].image + " width='48' height='48'>");
            }
            else
            {
                hu += "No image";
            ...
            if(j.content != "None")
            {
                ...
                for(var q=0; q<j.content.length; q++)
                {
                    ...
                    if(cam_id==1)
                    {
                        ...
                        ctx.fillStyle = 'black';
                        ctx.fillText("server fps: " + j.fps + ", client fps: " + (1000.0/timediff).toFixed(2) , 30, 100);
                    }
                }
...
function OpenLearnPersonWindow()
{
    ...
    hd += "<button type='button' onclick='Hide_Selected_Person(" + stun_result_table_length + ")'>Hide selected person</button> <br>"
    hd += "<table border='1'>";
