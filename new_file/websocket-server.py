...
def matching(bb, bbs_ref, identitylist, bbs_ref_isused, dist_thd):
    min_dist = 9999
    index = -1
    if len(bbs_ref)==0:
    ...
    for idx, bbb in enumerate(bbs_ref):
        ...
        if bbs_ref_isused[idx] == 0 and dist < min_dist and dist < dist_thd and identitylist[idx] != -99:
            min_dist = dist
            index = idx
...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, payload, isBinary):
        ...
        if msg['type'] == "ALL_STATE":
            ...
            print("A new person is added! [{}]".format(newpersonname))
            print("==================================================================")

        elif msg['type'] == "EDIT_PERSON_NAME":
            identity = msg['identity']
            oldpersonname = self.people[identity]
            newpersonname = msg['val'].encode('ascii', 'ignore')
            self.people[identity] = newpersonname
            print("A person name is changed! [{} -> {}]".format(oldpersonname, newpersonname))
            print("==================================================================")            
    ...
    def processFrame(self, dataURL, identity):
        ...
        if self.counter % 2 ==0:
            ...
            for idxx, bb in enumerate(bbs):
                ...
                matchingresult = matching(bb, self.prev_bbs, self.prev_identity, prev_bbs_isused, dist_thd)
                if not self.training and not self.stop_all_tracking and len(self.prev_bbs)>0 and len(self.prev_identity)>0 and len(self.prev_rep)>0 and matchingresult[0] >= 0:
