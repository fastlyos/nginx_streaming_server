...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def processImgToEncode(self, cam_id):
        ...
        while True:
            ...
            if not self.bufferframe[cam_id].empty():
                ...
                self.bufferframe_toprocess[cam_id].put(frame)
            ...
            if self.trainingnumber > 0:
                ...
                for zz in range(0, self.trainingnumber):
                    ...
                    if ctx is None:
                        ...
                        self.trainingContent[zz] = [str(x) for x in alignedFace.flatten()]   
                        #self.trainingContent[zz] = [str(x) for x in cv2.resize(alignedFace, (0,0), fx=0.5, fy=0.5).flatten()]   
                        ...
                        print("One face (index: {}) of identity {} is processed.".format(zz, identity))
    ...
    def Determine_identity_by_rep(self, cam_id, rep):
        ...
        if len(self.people) == 0:
            ...
            if self.svm:
                ...
                print("[cam{}] prob of each class: {}".format(cam_id, ' '.join(str(e) for e in prob)))
            ...
            if identity >= 0:
                if self.mean and self.std:
                    ...
                    print("[cam{}] This bb rep distance to class centre is {}".format(cam_id, dist_to_center))
                    ...
                    if BestMatchSimilarityScore > 0.4:
                        ...
                        for idx, val in enumerate(self.std[identity]):
                            ...
                            if ratio > ratio_thd:
                                identity = -1
                                print("[cam{}] Diff > {}*Std, not so sure is one of the trained person, treat as unknown".format(cam_id, ratio_thd))
                                break
    ...
    def matching(self, cam_id, bb, bbs_ref, identitylist, bbs_ref_isused):
        min_dist = 9999
        index = -1
        if len(bbs_ref)==0:
        ...
        if index >= 0:
            ...
            if bb_ratio < 0.6 or bb_ratio > 1.0/0.6:
                index = -1
                print("[cam{}] bb_ratio exceed limit, fail!".format(cam_id))
    ...
    def onMessage(self, payload, isBinary):
        ...
        if self.counter < 50 or self.counter % 30 == 0:
        ...
        if msg['type'] == "ALL_STATE":
            ...
            aug_ratio = 1
            if isWithAug and len(img_list_dataurl)<25 :
                aug_ratio = 50/len(img_list_dataurl)
            ...
            for zz in range(0, len(decoded_imgs)):
                ...
                if augmentation_num > 0:
                    images_aug = seq.augment_images([decoded_imgs[zz]]*augmentation_num)

                images_aug.insert(0, decoded_imgs[zz])  # insert the original photo to the front
    ...
    def loadState(self, jsImages, training, jsPeople, jsIdentity):
        ...
        for jsImage in jsImages:
            ...
            if rep == "0":
                ...
                ratio_y = 96.0/photo.shape[0]
                resizedphoto = cv2.resize(photo, (0,0),fx=ratio_x, fy=ratio_y)                     
                rep = cpunet.forward(resizedphoto)
    ...
    def processFrame(self, cam_id):
        ...
        while True:
            ...
            if self.bufferframe_toprocess[cam_id].empty():
                ...
                rgbFrame = self.bufferframe_toprocess[cam_id].get_nowait()
            ...
            for idxx, bb in enumerate(bbs):
                ...
                max_tracking_count = 5
                matchingresult = self.matching(cam_id, bb, self.prev_bbs[cam_id], self.prev_identity[cam_id], prev_bbs_isused)
                if (not self.training and len(self.prev_bbs[cam_id])>0 and 
                    ...
                    (self.prev_identity[cam_id][matchingresult[0]] >= 0 or (self.prev_identity[cam_id][matchingresult[0]] == -1 and self.counter % max_tracking_count !=0))):
                    ...
                    if self.unknowntraining and identity == -1:
                        ...
                        alignedFace = align.align(args.imgDim, rgbFrame, bb, landmarkIndices=openface.AlignDlib.OUTER_EYES_AND_NOSE) 
                ...
                #when tracking fails or if it is in active learning mode
                    ...
                    if not self.training and len(self.prev_bbs[cam_id])>0 and matchingresult[0] < 0:
                        print("[cam{}] Tracking fail, tracking dist is {}, do normal flow".format(cam_id, matchingresult[1]))
