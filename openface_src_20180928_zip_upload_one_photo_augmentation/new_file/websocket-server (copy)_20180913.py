...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    def __init__(self):
        ...
        self.ui_size_x = 816 #816 #160 #384 #816
        self.ui_size_y = 459 #459 # 90 # 216 #459
    ...
    def processImgToEncode(self, cam_id):
        ...
        while True:
            ...
            if not self.bufferframe[cam_id] is None:
                ...
                cv2.putText(sendimg, str(datetime.datetime.now()), (100, 95), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, 2)
    ...
    def queryFrame(self, cam_id, path):
        ...
        while True:
            ...
            if self.cap[cam_id].isOpened():
                ...
                start = time.time()
                #for kk in range(0,6):
                ret, self.bufferframe[cam_id] = self.cap[cam_id].read()
                ...
                if self.bufferframe[cam_id] is None:
    ...
    def onMessage(self, payload, isBinary):
        ...
        if msg['type'] == "ALL_STATE":
            ...
            cam_id = msg['id']
            if not self.processed_msg[cam_id] is None:
                self.sendMessage(json.dumps(self.processed_msg[cam_id]))
                print("Sent cam_id {} frame info to client".format(cam_id))
            else:
                time.sleep(0.005)
                msg = {
                    "type": "ANNOTATED",
    ...
    def processFrame(self, cam_id):
        ...
        while True:
            ...
            rgbFrame = None
            if self.bufferframe[cam_id] is None or self.bufferframe_gray[cam_id] is None:
                print("self.bufferframe[{}] is None, skip frame".format(cam_id))
                # msg = {
                #     "type": "SKIP"
                # #     "content": "None",
                # #     "fps": 1,
                ...
                rgbFrame = self.bufferframe[cam_id].copy()
            ...
            faces = cv2gpu.find_faces(self.bufferframe_gray[cam_id])
                #self.zzzjpg_mutex.release()
            ...
            if args.verbose:
                print("One frame took {} seconds. fps= {}".format(self.lastframetime[cam_id], 1/self.lastframetime[cam_id]))       
