...
def matching(bb, bbs_ref, identitylist, bbs_ref_isused):
    min_dist = 9999
    index = -1
    if len(bbs_ref)==0:
    ...
    for idx, bbb in enumerate(bbs_ref):
        ...
        if bbs_ref_isused[idx] == 0 and dist < min_dist and identitylist[idx] != -99 and dist > 0.0:
            min_dist = dist
            index = idx
    ...
    dist_thd = (bb.right()-bb.left())*0.45
    print("dist_thd: {}".format(dist_thd))

    if min_dist > dist_thd:
        index = -1
    ...
    def processFrame(self, dataURL, identity):
        ...
        if self.counter % 2 ==0:
            ...
            for idxx, bb in enumerate(bbs):
                ...
                matchingresult = matching(bb, self.prev_bbs, self.prev_identity, prev_bbs_isused)
                if (not self.training and not self.stop_all_tracking and len(self.prev_bbs)>0 and 
