...
bool FaceDetector::Detect(cv::Mat frame, //char* image_as_str,
                          std::vector<cv::Rect>& face_rects)
{
  // setting 0 return a grayscale image
  //cv::Mat frame = cv::imread(image_file, 0);

  //std::string strimg(image_as_str);
  //std::vector<uchar> data(strimg.begin(), strimg.end());
  ...
  if (gpu_)
  {
    //std::cout << "into if(gpu_)" << std::endl;
      ...
      cv::Mat faces_cpu;
      int num_faces = cascade_gpu.detectMultiScale(frame_gpu, faces_gpu, 1.05, 2, cv::Size(72, 72));
