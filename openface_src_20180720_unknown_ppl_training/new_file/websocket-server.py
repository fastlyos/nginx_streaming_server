...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def loadState(self, jsImages, training, jsPeople, jsIdentity):
        ...
        for jside in jsIdentity:
            self.identity_ofppl.append(jside.encode('ascii', 'ignore'))
    ...
    def trainFace(self):
        ...
        if d is None:
            ...
            self.svm = GridSearchCV(SVC(C=1, probability=True, verbose=True), param_grid, cv=5).fit(X, y)
    ...
    def processFrame(self, dataURL, identity):
        ...
        if self.counter % 2 ==0 or self.training:
            ...
            if (not self.training or self.training and self.unknowntraining ) and len(bbs) > 0:
