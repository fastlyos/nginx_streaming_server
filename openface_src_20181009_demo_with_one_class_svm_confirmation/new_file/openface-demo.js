...
var zwidth = 1280; //448; //640; //800 //1280
var zheight = 720; //252; //360; // 450  // 720
...
var unmatched_result_max_show = 500;
    ...
    for (var i = 0; i < len; i++)
    {
        ...
        if (info[i]>0)
        {
            ...
            for(var kz=0; kz<pic_to_show; kz++)
            {
                ...
                h += "<img src=" + images[imgIdx].image + " width='48' height='48' title='Identity index: " + identity_ofppl[i] + "'>";
            }
            h += "<br>";
        }
    }
...
function redrawResultTable()
{
    ...
    for(var p =result_table.length-1; p>=0; p--)
    {
        ...
        {
            hu += "<tr><td>";
            hu += ( "<img src=" + result_table[p].displayimage + " width='48' height='48'>");
            hu += "</td><td>";
        ...
        {
            hd += "<tr><td>";
            hd += ( "<img src=" + result_table[p].displayimage + " width='48' height='48'>");
            hd += "</td><td>";
            hd += "Unknown<br>(id:" + p + ")";
    ...
    socket.onmessage = function(e)
    {
        ...
        else if (j.type == "ANNOTATED")
        {
            ...
            if(j.learnmsg != "None")
            {
                console.log("receive new image...");
                images.push({
                    hash: j.learnmsg.hash,
                    identity: j.learnmsg.identity,
                    image: getDataURLFromRGB(j.learnmsg.content),
                    representation: j.learnmsg.representation
                });
                redrawPeople();
            }
            //var startTime, endTime;
            ...
            if(j.content != "None")
            {
                ...
                for(var q=0; q<j.content.length; q++)
                {
                    ...
                    for(var z=0; z<j.content[q][1].length; z++)
                    {
                        ...
                        {
                            coloridx = identity_ofppl.findIndex(k => k==j.content[q][1][z][5])+1;
                        }
                        ...
                        ctx.rect(j.content[q][1][z][1]*ratio,j.content[q][1][z][2]*ratio,j.content[q][1][z][3]*ratio,j.content[q][1][z][4]*ratio);
                        ctx.stroke();
        
                        //Draw the name of the detected person
                        ctx.font = "bold 35pt Calibri";
                        ctx.fillStyle = colorname[coloridx%7];
                        ctx.fillText(j.content[q][1][z][0], j.content[q][1][z][1]*ratio, j.content[q][1][z][2]*ratio-10);
                        ...
                        if(j.content[q][1][z][6])
                        {
                            ...
                            var imgIdx = findImageByIdentity(j.content[q][1][z][5]);
                            var rec_img = 0;
                            if (j.content[q][1][z][5] >= 0) rec_img = images[imgIdx].image
        
                            var nname = "na";
                            if (j.content[q][1][z][5] >= 0)
                            {
                                var pplidx = identity_ofppl.findIndex(k => k==j.content[q][1][z][5]);
                                nname = people[pplidx];
                            }
...
function OpenLearnPersonWindow()
{
    ...
    var hd = "<font size=5>Unknown person list</font> <br><br>";

    hd += "Name: <input type='text' id='learn_name' size=15> <br>";
    hd += "<button type='button' onclick='Select_All_to_Learn(" + stun_result_table_length + ")'>Select All</button> "
    hd += "<button type='button' onclick='UnSelect_All_to_Learn(" + stun_result_table_length + ")'>Unselect All</button> <br>"
    hd += "<table border='1'>";
    ...
    hd += "<input type='checkbox' id='LearnWithAug' checked='checked'> Learn with augmentation<br>";
...
{
    console.log("updateIdentity " + hash);
